...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%define_constants;
global REF PQ PV BUS_I BUS_TYPE F_BUS T_BUS BR_R BR_X BR_B GEN_BUS...
    GEN_STATUS PMAX PMIN QMAX QMIN VG MBASE PD QD BASE_KV VM VA PG QG;
BUS_I = 1;
BUS_TYPE = 2;
F_BUS = 1;
T_BUS = 2;
...
casefileName = 'C:/Users/chakr138/Aelios/situationalAwareness/dataset/case33bw_recons';
%mpc = loadcase(casefileName);
mpc_bus = importdata('.\dataset\mpc_bus.txt');
mpc_gen = importdata('.\dataset\mpc_gen.txt');
mpc_branch = importdata('.\dataset\ppc_branch.txt');
mpc = struct('version', '2', 'baseMVA', 100, 'bus', mpc_bus, 'gen', mpc_gen, 'branch', mpc_branch);
...
function network = getAgentNetwork(mpc, bus, default_ip, port_offset)
    %define_constants;
    global USECASE GRAPH_TYPE REF PQ PV BUS_I BUS_TYPE F_BUS T_BUS BR_R BR_X BR_B GEN_BUS...
    GEN_STATUS PMAX PMIN QMAX QMIN VG MBASE PD QD BASE_KV VM VA PG QG;
    ...
    network = struct('type', GRAPH_TYPE, 'to', table(id, ip, port, access_code,...
