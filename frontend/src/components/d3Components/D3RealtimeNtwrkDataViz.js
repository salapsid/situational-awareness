...
const createZoomableLineChart = (elem, pathDataArray, tooltipDiv) => {
  ...
  pathSvgGroup
    ...
    .on("mouseover", function (event, d) {
      ...
      console.log(`sec:  ${convToDate(d.x).getMinutes()} :${convToDate(d.x).getSeconds()}`);
      tooltipDiv.html(`

                    <p style="margin:0"> x ${convToDate(d.x).getMinutes()} :${convToDate(d.x).getSeconds()} </p>
                    <p style="margin:0"> y ${d.y.toFixed(5)}</p>
...
const D3RealtimeNtwrkDataViz = ({ ntwrkData, dataPkt, statusChangeData }) => {
